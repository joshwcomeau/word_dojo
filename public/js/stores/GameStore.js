...
var _moveStates = {
  ...
  TILE_DEACTIVATED: "TILE_DEACTIVATED",  // A single tile deactivated; you undid last move.
  WORD_DEACTIVATED: "WORD_DEACTIVATED",  // you clicked a tile in your word and deactivated it.
  NEW_WORD_STARTED: "NEW_WORD_STARTED"   // you clicked a non-neighboring cell and started a new word.
}
...
function clickTile(column, row) {
  ...
  if ( clickedTile.active ) {
    ...
    if ( clickedTile !== _active[_active.length-1] ) {
      _recentMoveState = _moveStates.WORD_DEACTIVATED;
      setAllToInactive();
    } else {
      _recentMoveState = _moveStates.TILE_DEACTIVATED;
    ...
    if ( _active.length && _.intersection(neighbors, _active).length === 0 ) {
      _recentMoveState = _moveStates.NEW_WORD_STARTED;
      setAllToInactive();
    } else {
      _recentMoveState = _moveStates.TILE_ACTIVATED;
    }
...
function evaluateWord() {
  ...
  if ( validWord ) {
    ...
    _recentMoveState = _moveStates.WORD_ACCEPTED;
  } else {
    _recentMoveState = _moveStates.WORD_REJECTED;
  }
